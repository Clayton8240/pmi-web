...
class PDFService:
    ...
    def _desenhar(self, c: canvas.Canvas, dados: Dict, itens: List[Dict],
        ...
        def draw_cell_wrapped(x, y, w, h, lines, align="L"):
            ...
            p.rect(x + 1, y + 0.5, w - 2, h - 1)
            c.clipPath(p, stroke=0, fill=0)
            c.setFont(ITEM_FONT, ITEM_SIZE)
            n = len(lines)
            block_h = n * ITEM_LINE_H
            usable_h = h - ITEM_PAD
            if block_h <= usable_h:
                # cabe: centraliza verticalmente
                start_y = y + (h + block_h) / 2 - ITEM_LINE_H * 0.75
            else:
                # não cabe: começa do topo; clipPath corta o excesso
                start_y = y + h - ITEM_PAD / 2 - ITEM_LINE_H * 0.85
            for line in lines:
                if align == "C":
        ...
        # Pré-calcula linhas e altura natural de cada item
        items_to_draw = itens[:12]
        items_data = []
        for item in items_to_draw:
            ...
            all_lines = [wrap_text(vals_row[i], ITEM_FONT, ITEM_SIZE, cw[i] - 6) for i in range(len(cw))]
            max_lines = max(len(ls) for ls in all_lines)
            natural_h = max(5 * mm, max_lines * ITEM_LINE_H + ITEM_PAD)
            items_data.append((all_lines, natural_h))

        # Espaço disponível para itens + linha de total (3 mm de margem de segurança)
        items_area = y - foot_h - 3 * mm
        total_natural = sum(d[1] for d in items_data) + total_h

        if total_natural <= items_area:
            row_heights = [d[1] for d in items_data]
        else:
            # Comprime uniformemente para caber todo o conteúdo
            avail_rows = max(0.0, items_area - total_h)
            n = len(items_data)
            uniform_h = max(5 * mm, avail_rows / n) if n > 0 else 5 * mm
